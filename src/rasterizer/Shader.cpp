...
Shader::Shader() :
    view_pos(vec3f_t::Zero()),
    model_mat(mat4f_t::Identity()),
    view_mat(mat4f_t::Identity()),
    projection_mat(mat4f_t::Identity())
{
}

vec3f_t Shader::getViewPos()
{
	return view_pos;
}

mat4f_t Shader::getModelMat()
{
	return model_mat;
}

mat4f_t Shader::getViewMat()
{
	return view_mat;
}

mat4f_t Shader::getProjectionMat()
{
...
void Shader::setViewPos(const vec3f_t& view_pos)
{
	this->view_pos = view_pos;
}

void Shader::setModelMat(const mat4f_t& model_mat)
{
	this->model_mat = model_mat;
}

void Shader::setViewMat(const mat4f_t& view_mat)
{
	this->view_mat = view_mat;
}

void Shader::setProjectionMat(const mat4f_t& projection_mat)
{
		...
		for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
			...
			for (size_t v = 0; v < fv; v++) {
				...
				if (idx.texcoord_index >= 0) {
					triangle.texcoords[v] = vec2f_t(
					    attrib.texcoords[2 * size_t(idx.texcoord_index) + 0],
					    attrib.texcoords[2 * size_t(idx.texcoord_index) + 1]);
				}

				// record colors
				triangle.colors[v] = vec3f_t(
...
vec3f_t Shader::phongShader(const ShaderInfo& shader)
{
...
vec3f_t Shader::textureShader(const ShaderInfo& shader)
{
