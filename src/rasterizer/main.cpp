...
void init()
{
	...
	mat = Geometry::translate(mat, vec3f_t(pipeline.getWidth() / 2.f - center.x(), pipeline.getHeight() / 2.f - center.y(), 0.f));
	mat = Geometry::scale(mat, vec3f_t(scale, scale, scale));
...
void tick()
{
	...
	mat = Geometry::rotate(mat, duration.count(), vec3f_t(0.f, 1.f, 0.f));
