...
void Rasterizer::clear(vec3f_t color)
{
...
void Rasterizer::setPixel(int x, int y, const vec3f_t& color)
{
...
void Rasterizer::setPixel(const vec3f_t& point, const vec3f_t& color)
{
...
void Rasterizer::drawTriangle(const triangle_t& triangle, ShaderInfo& shader_info)
{
	...
	for (int x = min_x; x < max_x + 1; x++) {
		for (int y = min_y; y < max_y + 1; y++) {
			...
			shader_info.normal = interpolate<vec3f_t>(alpha, beta, gamma, n[0], n[1], n[2]);
			shader_info.color = interpolate<vec3f_t>(alpha, beta, gamma, c[0], c[1], c[2]);
			shader_info.texcoord = interpolate<vec2f_t>(alpha, beta, gamma, t[0], t[1], t[2]);
...
void Rasterizer::drawLine(const std::array<vec3f_t, 2>& line, const vec3f_t& color)
{
	bool is_steep = false;
	auto begin = line[0];
	auto end = line[1];
...
void Rasterizer::drawPoint(const vec3f_t& point, const vec3f_t& color)
{
	int x = static_cast<int>(point.x());
	int y = static_cast<int>(point.y());
